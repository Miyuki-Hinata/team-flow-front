...
## 面接で説明できるポイント
- **薄いページコンポーネントの設計**：ページは「何を取得し、何を並べるか」だけ持つ。表示や状態管理は下位（Container / Filter / List / Card）が分担
- **`useNavigate` + Button の遷移パターン**：素の `<Link>` を安易にボタン風 styled にせず、Button コンポーネントの variant システムを1箇所に集約する判断
- **スコープ管理**：デザインが理想を提示していても、ドメインモデルに存在しない属性（status）は不用意に追加せず、別Issue に切り出す判断
