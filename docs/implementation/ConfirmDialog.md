...
## 面接で説明できるポイント
- **既存資産の合成で作る**：Modal（開閉）× Button（見た目）× ConfirmDialog（確認の意味づけ）の3層。それぞれが単一責任で、ConfirmDialog 自身はごく薄い。
- **危険な操作の UI 配慮**：実行ボタンを赤（danger）にする、キャンセルと実行のハンドラを分ける、という「誤操作を防ぐ」設計判断を説明できる。
- **デフォルト props の使い分け**：頻出ケース（削除）はデフォルトで最短、それ以外は上書き、という API の作り方。`message` だけ必須にした理由も含めて語れる。
- **既存挙動を再実装しない**：Escape / オーバーレイクリックの閉じる挙動を Modal から継承し、車輪の再発明を避けた点。
