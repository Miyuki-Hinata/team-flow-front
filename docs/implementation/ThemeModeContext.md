...
## 面接で説明できるポイント
- **Provider の 2 段包み（この Context + styled-components 内包）**：Provider を分けず 1 つにまとめることで、呼び出し側（main.tsx）は「モードが切り替わる」ことだけを気にすればよくなる。theme の受け渡しは完全にカプセル化。
- **初期値取得を `useState` の初期化関数に**：`useState(getInitialMode)` と書くことで、コンポーネントの再レンダーごとに `localStorage.getItem` が呼ばれるのを防ぐ（初回のみ実行）。
