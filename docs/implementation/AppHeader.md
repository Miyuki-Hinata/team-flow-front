...
## 面接で説明できるポイント
- **既存ロジックを完全温存しつつ、UI 表現を全面刷新**：認証・ログアウトのフローに1ミリも触れず、外側だけデザインに寄せた refactor と redesign の線引き。
- **ドロップダウンの外クリック閉じ**：`useRef` で「メニューの内側」を掴み、`contains` で外側判定するイディオム。既存 `ui/UserMenu` の同パターンから流用した。
- **stacking context に注意**：モーダルを `position: sticky` の子孫に置くと想定通りに前面に来ないため、`<>` で Header と兄弟に配置している。
- **Sidebar とラベルを一元化**：`NAV_ITEMS` を共有する設計判断。同じ画面のラベルが2箇所でズレる事故を防いでいる。
