...
## 面接で説明できるポイント
- **状態のリフトアップ**：選択状態を子に持たせず親へ集約した理由（複数選択の整合性・単一情報源）。制御コンポーネントの考え方。
- **`<label>` 合成でクリック領域を広げる**：JS を足さずネイティブ挙動でカード全体を選択可能にした。a11y も標準のまま。
- **合成による再利用**：PatientCard を改変せず「表示」と「選択」を分離して重ねた。関心の分離とDRYの両立。
