...
## 面接で説明できるポイント
- **ナビ定義の単一情報源**：`NAV_ITEMS` を Sidebar／AppHeader（パンくず）の両方が参照。ラベル変更のコストを最小化。
- **アクティブ判定の「具体優先」ロジック**：前方一致の落とし穴（複数項目が同時に光る）を、より長いパスがマッチするなら短い方を降ろす、という2段判定で解消。
- **currentColor でアイコン色を親に委譲**：色切替コードを重複させない CSS のイディオム。
- **暗背景専用色の扱い**：theme トークンに無い値（`#1A2348`）を、README の明示定義を根拠に採用したという判断基準を説明できる。
