...
## 面接で説明できるポイント
- **下書き状態と確定状態の分離**：ローカルの選択下書き（キャンセルで破棄）とサーバ確定状態（onSaved で親へ）を分け、「保存するまで反映されない」UXを実現した理由を説明できる。
- **集合置換PUTとの噛み合わせ**：UIが最終的な選択集合を持ち、それをそのまま送るので、冪等なPUTと素直に対応する（差分計算不要）。
- **関心の分離**：Picker＝選択と保存、ページ＝確定状態の保持。責務境界を `onSaved` の1点に絞った。
- **`Set` を使った不変更新**：`new Set(prev)` で作り直す React の不変更新パターン。
- **既存の Loading/EmptyState 規約への準拠**：`T[] | null` で未取得を表す一貫性。
