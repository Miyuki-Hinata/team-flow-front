...
## 面接で説明できるポイント
- **選択肢をデータで受ける設計の利点**：描画ロジックを1箇所に集約でき、選択肢の追加・変更が配列の編集だけで済む。テストもデータ差し替えで容易。
- **placeholder＝空 value の意味**：未選択を空文字で表す標準作法。フォーム状態と `required` の相性がよい。
- **独自 props を rest に混ぜない理由**：カスタム props を DOM 要素へ流すと React が不明な属性として警告する。分割代入で明示的に抜くことで、DOM には有効な属性だけが渡る。
- **Input との視覚的統一**：同じトークンを使うことで、フォーム内で入力欄とセレクトの高さ・枠・focus 挙動が揃い、UI の一貫性（電子カルテUIへのアンチテーゼ）を保てる。
- **拡張の余地**：PrioritySelect / TaskStatusSelect はこの Select をラップして options を固定するだけで作れる（値→tone/label のマッピングをラッパー側に閉じ込める）。
