...
> ⚠️ **このドキュメントは UI 整備（Issue #18）に着手する前、2026年7月時点のスナップショットです。**
> 「素の `<button>`/`<input>` がどこに重複しているか」を洗い出し、共通コンポーネント化の優先順位を
> 決めるために作成した監査記録で、**ここで挙げた指摘事項は対応済み**です（結果は `docs/implementation/` の
> 各ドキュメントを参照）。ファイルパス・行番号は当時のものなので、現在のコードとは一致しません。
> リファクタ前の状態と着手判断の記録として残しています。
